namespace vpb.app.business.ktnb.CoreData
{
    ...
    internal class db_Role_Project : ADBManager
    {
        private Database _db;
        private static db_Role_Project mvarInstance;
        private UserContext mvarUserContext;

        public db_Role_Project(UserContext usrCTX)
        {
        ...
        public db_Role_Project(UserContext usrCTX, string databaseName)
        {
            this.mvarUserContext = usrCTX;
            if (this.mvarUserContext == null)
            {
        ...
        public DataSet getApplicationByRole(string RoleID, string Condition)
        {
            ...
            string commandText = ("SELECT" + str) + " FROM T_PROJECT_ROLE T1 LEFT JOIN T_PROJECT T2 ON T1.FK_APPLICATIONID=T2.PK_APPLICATIONID WHERE 1=1" + str3;
            try
            {
        ...
        public override DataSet getByID(string ID, string Query)
        {
            try
            {
                ...
                {
                    commandText = "SELECT *";
                    commandText = (commandText + " FROM T_PROJECT") + " WHERE PK_APPLICATIONID='" + ID + "'";
                }
                else
                {
                    commandText = ("SELECT " + Query + " FROM T_PROJECT") + " WHERE PK_APPLICATIONID='" + ID + "'";
                }
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
            catch
        ...
        public DataSet getComponentByApplication(string ApplicationID, string RoleID)
        {
            ...
            string commandText = (("SELECT" + str + " FROM T_COMPONENT_ROLE T1") + " LEFT JOIN T_COMPONENT T2 ON T1.FK_COMPONENTID=T2.PK_COMPONENTID" + " LEFT JOIN T_PROJECT T3 ON T2.FK_APPLICATIONID=T3.PK_APPLICATIONID") + " WHERE 1=1" + str3;
            try
            {
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
        ...
        public override DataSet getList(string Condition, string Query)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(Query))
                {
                    commandText = "SELECT * FROM T_ROLE_PROJECT WHERE 1=1 ";
                }
                else
                {
                    commandText = "SELECT " + Query + " FROM T_ROLE_PROJECT WHERE 1=1 ";
                }
                if (!string.IsNullOrEmpty(Condition))
                {
                    commandText = commandText + Condition;
        ...
        public static db_Role_Project Instance(UserContext usrCTX)
        {
            if (mvarInstance == null)
            {
                mvarInstance = new db_Role_Project(usrCTX);
            }
            return mvarInstance;
        }
