namespace vpb.app.business.ktnb.CoreData
{
    ...
    using vpb.app.business.ktnb.CoreInterface;

    internal class db_Application : ADBManager
    {
        private Database _db;
        private static db_Application mvarInstance;
        private UserContext mvarUserContext;

        public db_Application(UserContext usrCTX)
        {
        ...
        public db_Application(UserContext usrCTX, string databaseName)
        {
            this.mvarUserContext = usrCTX;
            if (this.mvarUserContext == null)
            {
        ...
        public override int addnewDataSet(DataSet ds)
        {
            ...
            DbCommand storedProcCommand = this._db.GetStoredProcCommand("DMS_AddNew_Application");
            this._db.AddInParameter(storedProcCommand, "PK_APPLICATIONID", DbType.Guid, "PK_APPLICATIONID", DataRowVersion.Current);
            ...
            this._db.AddInParameter(storedProcCommand, "FK_PROJECTID", DbType.Guid, "FK_PROJECTID", DataRowVersion.Current);
        ...
        public override int delete(string Condition)
        {
            ...
            commandText = "DELETE FROM T_APPLICATION WHERE 1=1 ";
        ...
        public override int deleteByID(string ID)
        {
            ...
            this._db.AddInParameter(storedProcCommand, "PK_APPLICATIONID", DbType.String, ID);
            this._db.AddOutParameter(storedProcCommand, "RETURNVALUE", DbType.Int32, 4);
        ...
        {
            string str3;
            string str = " T2.PK_APPLICATIONID, T2.NAME, T2.DESCRIPTION, T2.ORDERINDEX, T2.LEFTRIGHTPOSITION, T2.URL, T2.IMAGENAME";
            if (!string.IsNullOrEmpty(Condition))
            {
            ...
            string commandText = ("SELECT" + str) + " FROM T_APPLICATION_ROLE T1 LEFT JOIN T_APPLICATION T2 ON T1.FK_APPLICATIONID=T2.PK_APPLICATIONID WHERE 1=1" + str3;
            try
            {
        ...
        {
            string str3;
            string str = " DISTINCT T2.PK_APPLICATIONID, T2.NAME, T2.DESCRIPTION, T2.ORDERINDEX, T2.LEFTRIGHTPOSITION, T2.URL, T2.IMAGENAME";
            if (!string.IsNullOrEmpty(Condition))
            {
                str3 = " AND FK_ROLEID IN (SELECT FK_ROLEID FROM T_USER_IN_ROLE WHERE FK_USERID='" + UserID + "')" + Condition + " ORDER BY ORDERINDEX";
            }
            else
            {
                str3 = " AND FK_ROLEID IN (SELECT FK_ROLEID FROM T_USER_IN_ROLE WHERE FK_USERID='" + UserID + "') ORDER BY ORDERINDEX";
            }
            string commandText = ("SELECT" + str) + " FROM T_APPLICATION_ROLE T1 INNER JOIN T_APPLICATION T2 ON T1.FK_APPLICATIONID=T2.PK_APPLICATIONID WHERE 1=1" + str3;
            try
            {
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
        ...
        public override DataSet getByID(string ID, string Query)
        {
            try
            {
                ...
                {
                    commandText = "SELECT *";
                    commandText = (commandText + " FROM T_APPLICATION") + " WHERE PK_APPLICATIONID='" + ID + "'";
                }
                else
                {
                    commandText = ("SELECT " + Query + " FROM T_APPLICATION") + " WHERE PK_APPLICATIONID='" + ID + "'";
                }
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
            catch
        ...
        public DataSet getComponentByApplication(string ApplicationID, string RoleID)
        {
            ...
            string commandText = (("SELECT" + str + " FROM T_COMPONENT_ROLE T1") + " LEFT JOIN T_COMPONENT T2 ON T1.FK_COMPONENTID=T2.PK_COMPONENTID" + " LEFT JOIN T_APPLICATION T3 ON T2.FK_APPLICATIONID=T3.PK_APPLICATIONID") + " WHERE 1=1" + str3;
            try
            {
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
        ...
        public override DataSet getEmpty(string Query)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(Query))
                {
                    commandText = "SELECT " + Query + " FROM T_APPLICATION WHERE 1=2";
                }
                else
                {
                    commandText = "SELECT * FROM T_APPLICATION WHERE 1=2";
                }
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
            catch
        ...
        public override DataSet getList(string Condition, string Query)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(Query))
                {
                    commandText = "SELECT * FROM T_APPLICATION WHERE 1=1 ";
                }
                else
                {
                    commandText = "SELECT " + Query + " FROM T_APPLICATION WHERE 1=1 ";
                }
                if (!string.IsNullOrEmpty(Condition))
                {
                    commandText = commandText + Condition;
        ...
        public DataSet getRoleOnApplication(string ID)
        {
            try
            {
                string commandText = "SELECT FK_ROLEID,NAME FROM T_APPLICATION_ROLE T1 LEFT JOIN T_ROLE T2 ON T1.FK_ROLEID=T2.PK_ROLEID WHERE FK_APPLICATIONID='" + ID + "' ORDER BY NAME";
                return this._db.ExecuteDataSet(CommandType.Text, commandText);
            }
            catch
            {
        ...
        public static db_Application Instance(UserContext usrCTX)
        {
            if (mvarInstance == null)
            {
                mvarInstance = new db_Application(usrCTX);
            }
            return mvarInstance;
        }
