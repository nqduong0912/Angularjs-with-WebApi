function duyetKeHoachNamController($scope, $route, $routeParams, $location, $http, blockUI, toastr) {
    $scope.working = false;
    ...
    $scope.initFunc = function (banGDKhoiId, banKSId) {
        ...
        blockUI.start();
        $scope.roles.isBanGDKhoi = userContext.isInGroup(banGDKhoiId);
        $scope.roles.isBanKS = userContext.isInGroup(banKSId);
