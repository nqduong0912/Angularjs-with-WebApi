<%@ Page Title="" Language="C#" MasterPageFile="~/Share/Wsp.master" AutoEventWireup="true"
    CodeBehind="BLD_Duyet.aspx.cs" Inherits="VPB_TDHS.Modules.DMS.BLD_Duyet" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function PheDuyetTuChoi(DotKiemToanID, hanhdong) {
            var sHanhDong = (hanhdong == 'pheduyet') ? "phê duyệt" : "từ chối";
            if (!window.confirm('Bạn có muốn ' + sHanhDong + ' ?'))
                return false;
            var url = "BLD_Duyet.aspx";
            var dotkt = DotKiemToanID;
            var query = "act=PheDuyetTuChoiHSRR";
            query += "&hanhdong=" + hanhdong + "&dotkt=" + DotKiemToanID;
            StartProcessingForm("");
            $.ajax({
                type: "POST",
                url: url,
        ...
        function Duyet_XuatBBKT(doankt, dotkt) {
            url = "ReviewDotKiemToan.aspx?act=bld_bbkt&doankt=" + doankt + "&dotkt=" + dotkt;
            window.location.href = url;
        }
    </script>
</asp:Content>
