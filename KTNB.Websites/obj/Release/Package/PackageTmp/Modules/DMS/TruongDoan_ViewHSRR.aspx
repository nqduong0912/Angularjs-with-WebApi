<%@ Page Title="" Language="C#" MasterPageFile="~/Share/Wsp.master" AutoEventWireup="true"
    CodeBehind="TruongDoan_ViewHSRR.aspx.cs" Inherits="VPB_TDHS.Modules.DMS.TruongDoan_ViewHSRR" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContent" runat="server">
    ...
    <asp:ObjectDataSource runat="server" ID="ObjectDataSource1" SelectMethod="getListHoSoPhanTichChiTietByTruongDoan"
        TypeName="DataSource">
        <SelectParameters>
            <asp:Parameter Name="DotKiemToanID" Type="string" />
                            ...
                            <Columns>
                                <C1WebGrid:C1TemplateColumn Visible="false">
                                    <ItemTemplate>
                                        ...
                                        <asp:HiddenField runat="server" ID="hiddenNhomKT" Value='<%# DataBinder.Eval(Container.DataItem,"nhomkt")%>' />
                                    </ItemTemplate>
        ...
        <tr>
            <td>
                <asp:UpdatePanel runat="server" ID="updatepanel1" UpdateMode="Conditional">
                    <ContentTemplate>
                        <C1WebGrid:C1WebGrid runat="server" ID="dataCtrl" Width="100%" OnSortingCommand="dataCtrl_OnSorting"
                            DataSourceID="ObjectDataSource1" OnItemCreated="dataCtrl_OnItemCreated" OnItemDataBound="dataCtrl_OnItemDataBound"
                            ...
                            <Columns>
                                ...
                                <C1WebGrid:C1BoundColumn SortExpression="mang_nghiep_vu" HeaderText="Mảng nghiệp vụ"
                                    DataField="mang_nghiep_vu">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            //do smt here
            $('input[name*="btnSAVE"]').css('width', '100px');
        });
        /*********************************************************/
        ...
        function KiemTraPTRRHetChua() {
            if (!window.confirm('Xin ban lãnh đạo duyệt hồ sơ rủi ro?'))
                return false;
            var DotKiemToanID = '<%=_dotkt%>';
            //            var nhomkt = '<%=_nhomkt%>';
            var url = "TruongDoan_ViewHSRR.aspx";
            var dotkt = DotKiemToanID;
            var query = "act=KiemTraPTRRHetChua";
            query += "&dotkt=" + DotKiemToanID;
            StartProcessingForm("");
            $.ajax({
                type: "POST",
                url: url,
        ...
        function XinDuyet(DotKiemToanID) {
            ...
            $.ajax({
                ...
                success: function (message) {
                    alert(message);
                    FinishProcessingForm();
                    url = "TruongDoan_XinDuyet.aspx?act=hsrr";
                    window.location.href = url;
                }
            });
        }
